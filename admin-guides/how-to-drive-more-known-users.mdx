...
Until the user opts to give you more information, this is the most you can know about the user — and this by design. Once the user does opt in to giving their email, Koala will record that email address and the browser session will be associated with that user going forward — even if it wasn't a formal "log in".

## Koala-specific suggestions
Koala offers some unique ways to better understand intent signals from known users:

**Set up intent signals happening in your application.** When a user signs up or logs in to your app, they are identifying themselves, so you'll be able to know exactly who is doing what in your app. We also find that some of the best intent is happening in your app — maybe you have a user paying you a little bit, but they are showing intent that they need something more sophisticated. 

*Note: leveraging this requires the Advanced Instrumentation.*
...
*Coming soon: email support@getkoala.com for early access.*
...
**Email gate your best content.** If you have a great piece of content that delivers lots of value on its own (eg., an E-Book, Industry Survey, or set of recipes), you may want to consider gating all or some of the content. Often it's a good idea to give a sample of the high-quality content for free and then put the email gate in a little later.
