...
<b>For this to achieve the desired outcome, you'll need to ensure that both the `email` property and the `profile_id` property are set (with those names!) and that the `profile_id` is read off the Koala cookie.</b>
