...
title: "HubSpot"
description:
  "Connect HubSpot's CRM to enable a two-way sync with Koala's data."
---
...
### What data will Koala sync?

Upon connecting Koala to HubSpot's CRM, Koala will sync all Accounts, Deals, and Contacts. Koala will automatically refresh HubSpot CRM every hour, so we always have fresh data about who owns which accounts and which accounts are in active deal cycles.
...
If Koala finds multiple Account objects matching a domain, we'll tiebreak according to the following rules:

1. Pick the account with most Deals
...
1. Import Accounts
2. Import Contacts
3. Claim Account (rewrite the account owner to be the current Koala user)
