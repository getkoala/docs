...
BigQuery have a schema detection logic that convert UUID type columns to `bytes` type ([more info](https://cloud.google.com/bigquery/docs/loading-data-cloud-storage-parquet#parquet_conversions)), which result in wrong result values when querying data from that columns. To avoid this, all you'll need is to convert the IDs columns using the [TO_HEX()](https://cloud.google.com/bigquery/docs/reference/standard-sql/string_functions#to_hex) function.

For example, selecting the IDs directly will return it as bytes (base64 encoded):
```
...
To fetch the real ID value, you need to convert it on query:
...
This conversion isn't needed on table joins. Use it just if you need to retrieve the real column value on your BigQuery views and queries.
