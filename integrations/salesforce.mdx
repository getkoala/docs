...
## Account Score Sync into Salesforce (Business Plan) - BETA
...
The integration sends scores to the CRM in groups of `~100_000` records, so you can see more than one entry per day in the Bulk Data jobs log (under `Setup > Environments > Jobs > Bulk Data Load Jobs`)
if there are more than that amount of scores to send.
